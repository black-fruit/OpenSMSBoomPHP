﻿<?php
// 密码 Cookie 加密盐
if(!defined('Qxys_ENCRYPT_SALT'))
    define('Qxys_ENCRYPT_SALT', 'Kgs$JC!V');
/**
 * 设置访问密码:qx-20719029
 *----插入要加密的文件中-----
 * require_once('jmapi.php');
 * Qxjmapi("qx-20719029");
 *--------插入代码结束-------
 ...
 */
function Qxjmapi($password, $pageid = 'default') {
    $pageid     = md5($pageid);
    $md5pw      = md5(md5($password).Qxys_ENCRYPT_SALT);
    $postpwd    = isset($_POST['pagepwd']) ? addslashes(trim($_POST['pagepwd'])) : '';
    $cookiepwd  = isset($_COOKIE['Qxys_encrypt_'.$pageid]) ? addslashes(trim($_COOKIE['Qxys_encrypt_'.$pageid])) : '';
    
    if($cookiepwd == $md5pw) return;    // Cookie密码验证正确
    
    if($postpwd == $password) {         // 提交的密码正确
        setcookie('Qxys_encrypt_' . $pageid, $md5pw, time() + 3600000, '/');
        return;
    }
?>
<html>
<head>
    ...
    *{font-family:"Microsoft Yahei",微软雅黑,"Helvetica Neue",Helvetica,"Hiragino Sans GB","WenQuanYi Micro Hei",sans-serif;box-sizing:border-box;margin:0px;padding:0px;font-size:14px;-webkit-transition:.2s;-moz-transition:.2s;-ms-transition:.2s;-o-transition:.2s;transition:.2s}
    ...
    .main{width:100%;max-width:500px;height:360px;padding:30px;background-color:#fff;border-radius:2px;box-shadow:0 10px 60px 0 rgba(29,29,31,0.09);transition:all .12s ease-out;position:absolute;left:0;top:0;bottom:0;right:0;margin:auto;text-align:center}
    .alert{width:80px}
...
<body>
    <div class="main">
        <svg class="alert" viewBox="0 0 1084 1024" xmlns="//www.w3.org/2000/svg" width="80" height="80">
            <defs></style></defs>
        ...
        <form action="" method="post" class="mk-side-form">
			...
			<p>本站仅供个人测试手机性能使用,严禁商业非法用途!否则后果自负。</p>
            <input type="password" name="pagepwd" placeholder="请输入访问密码查看，123456" required><button type="submit">提交</button>
            <?php if($postpwd): ?>
            <p id="pw-error">Oops!您输入的密码不对哦~</p>
            <script>setTimeout(function() {document.getElementById("pw-error").style.display = "none"}, 2000);</script>
            <?php endif; ?>
        </form>
        <a href="http://vip.aaaak.pw/" class="return-home" title="点击查看密码">- 点击查看密码 - </a>
    </div>
</body>
</html>
<?php
